...
class iplb (object):
  ...
  def _pick_server (self, server):
    """
    Pick a server for a (hopefully) new connection
    """
    #return random.choice(self.live_servers.keys()[:sleep.get_switch(dpid)])
  ...
  def _handle_PacketIn (self, event):
    ...
    #start sleep seq
    if self.first_packet_flag == 1:
      self.first_packet_flag  = 0
      sleep.schedule_sleep(10)
    ...
    if ipp.srcip in self.servers:
      ...
      if entry is None or entry.server not in self.live_servers:
        ...
        server = self._pick_server(sleep.get_server_count())
